...
- **Фронтенд:** HTML + CSS + JS-модули без сборки. Three.js лежит в проекте —
  ...
  системных).
- **Модель:** бесплатный облачный Groq (`openai/gpt-oss-120b`), локальный
...
В шапке разговора видно, кто отвечает: «Groq · gpt-oss-120b на связи».
...
С 16 августа 2026 модели Llama убраны из бесплатного тарифа — по умолчанию
используется `openai/gpt-oss-120b`. Лимиты считаются на организацию (все ключи
вместе), срабатывает тот, что кончился первым:

| | в минуту | в день |
| --- | --- | --- |
| Запросы | 30 | 1 000 |
| Токены (вход + выход) | 8 000 | 200 000 |

Одна реплика в «Тихом часе» — около 1,2 тыс. токенов в начале разговора и до
~2,9 тыс. в длинном (системный промпт + история до 24 сообщений + ответ).
Узкое место — **8 000 токенов в минуту на всех пользователей сразу**: это
3–6 ответов в минуту. При `LLM_FALLBACK=demo` лишние запросы получат
демо-ответы вместо ошибки. Точные лимиты аккаунта:
<https://console.groq.com/settings/limits>.

| Модель в `LLM_MODEL` | Когда брать |
| --- | --- |
| `openai/gpt-oss-120b` | по умолчанию: лучшее качество на русском |
| `openai/gpt-oss-20b` | быстрее, лимиты те же |

Если указанной модели у Groq нет, приложение само берёт доступную
(видно в `/api/health`).
...
Проект готов к Vercel без доработок: FastAPI находится сам (`app/main.py`),
ответы идут потоком, настройки лежат в `vercel.json`. Серверы Vercel стоят
за рубежом, поэтому Groq там работает без прокси.

### Через терминал (5 минут)

```bash
npm i -g vercel
cd ai-psiholog
vercel login
vercel                                   # первый деплой: на все вопросы Enter
vercel env add GROQ_API_KEY production   # вставить gsk_…
vercel env add LLM_FALLBACK production   # ввести: demo
vercel --prod                            # боевой адрес вида tihiy-chas.vercel.app
```

### Через GitHub (обновляется сам после каждого push)

1. Залейте папку в репозиторий GitHub (`.env` в него не попадёт — он в `.gitignore`).
2. <https://vercel.com/new> → **Import** этот репозиторий. Framework Preset
   определится как **FastAPI**, остальное не трогать.
3. **Environment Variables:** `GROQ_API_KEY` = ваш ключ, `LLM_FALLBACK` = `demo`.
4. **Deploy**.

`LLM_PROXY` на Vercel не нужен — не задавайте его там.

### Что учтено для serverless

- **История разговора.** Память функции не сохраняется между запросами, если
  они попали на разные экземпляры. Браузер присылает последние реплики вместе с
  сообщением, и сервер восстанавливает разговор, если своей сессии не нашёл.
  Итог разговора работает так же.
- **Время ответа** — до 60 с (`maxDuration`), с запасом для потока.
- **Статика** кешируется CDN Vercel на сутки после первой загрузки.
- **Секреты:** `.vercelignore` не даёт загрузить локальный `.env`.
- Лимит запросов считается внутри экземпляра — это защита от случайного
  перебора, а не жёсткая квота.

### Перед показом

Откройте сайт с телефона на мобильном интернете без VPN. Если в России адрес
`*.vercel.app` не открывается (такое бывает), подключите свой домен в
**Settings → Domains** или держите запасным вариантом локальный запуск
с `LLM_FALLBACK=demo`.
...
| `LLM_MODEL` | `openai/gpt-oss-120b` | Модель для `groq` / `openai` |
...
33 теста бэкенда без сети: поток и история, кризис, контекст настроения, итог,
...
полный разговор в демо-режиме, восстановление разговора из истории браузера
(как на Vercel после смены экземпляра).
...
├── vercel.json · .vercelignore · .python-version   деплой на Vercel
